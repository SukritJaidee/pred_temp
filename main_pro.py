...
for  i in tqdm(range(len(df))):
  ...
  try:
    ...
    try:
      ...
      # print('shape:', res_update.shape); print(display(res_update));
    except:
      print('error i =', i)
...
print('minutes:', min)
start_cp, end_cp = start_cp.strftime("%Y_%m_%d_%H_%M"), end_cp.strftime("%Y_%m_%d_%H_%M")
info = pd.DataFrame({'success':[str(success)], 'datetime':[str(start_cp)], 'start_compute':[str(start_cp)], 'end_compute':[str(end_cp)]})
info.to_csv('data'+start_cp+'_info.csv' ,encoding='utf-8-sig')
result.to_csv('data'+start_cp+'_data.csv' ,encoding='utf-8-sig')
