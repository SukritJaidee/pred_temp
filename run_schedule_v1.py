...
cols = ['lat', 'lon', 'latitude', 'longitude', 'elevation', 'distance', 'temp', 'dwpt', 'rhum', 'wdir', 'wspd', 'year',
        ...
        'pressureChange', 'pressureMeanSeaLevel', 'relativeHumidity', 'temperature', 'temperatureChange24Hour', 'temperatureMax24Hour',
...
def deploy_schedule(df, now_datetime, format_date, saved_lr, start_cp):
    ...
    for i in tqdm(range(len(df))):
        q1 = (df['lat_south'][i], df['lon_east'][i])
        try:
            if k == 0:
    ...
    info.to_csv('data_v1/'+'data_' + start_cp + '_info.csv', encoding='utf-8-sig')
    result.to_csv('data_v1/'+'data_' + start_cp + '_data.csv', encoding='utf-8-sig')
    return True
